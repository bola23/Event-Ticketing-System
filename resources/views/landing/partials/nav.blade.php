...
<header x-data="{ open: false }" class="fixed top-[3px] inset-x-0 z-50 flex items-center justify-between gap-4 px-5 md:px-16 py-5 bg-ccs-black/80 backdrop-blur border-b border-white/10">
    ...
    <div class="flex items-center gap-3 shrink-0">
        ...
        <a href="{{ route('speaker-requests.create', $event) }}" class="hidden sm:inline-flex px-5 py-2.5 rounded-md border border-white/20 text-sm font-bold whitespace-nowrap transition-colors hover:bg-white/10">{{ __('Become a Speaker') }}</a>
        <a href="{{ route('sponsor-requests.create', $event) }}" class="hidden sm:inline-flex px-5 py-2.5 rounded-md border border-white/20 text-sm font-bold whitespace-nowrap transition-colors hover:bg-white/10">{{ __('Become a Sponsor') }}</a>
